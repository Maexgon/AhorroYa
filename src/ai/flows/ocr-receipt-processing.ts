...
const ProcessReceiptInputSchema = z.object({
  ...
  base64Contents: z.array(z.string()).describe("An array of Base64 encoded strings of the receipt images/PDF. Must include MIME type (e.g., 'data:image/png;base64,...')."),
...
const processReceiptPrompt = ai.definePrompt({
  ...
  prompt: `You are an expert AI specializing in extracting information from Argentinian receipts and categorizing expenses. The user may upload multiple images for a single long receipt.

Your tasks are:
1.  Analyze all the provided receipt document(s) together as one single receipt and extract key information:
...
const processReceiptFlow = ai.defineFlow(
  ...
  async input => {
    ...
    if (!input.base64Contents || input.base64Contents.length === 0) {
        console.error('[FLOW] No content provided.');
        throw new Error('No receipt content provided. The base64Contents array is empty.');
    }
    ...
    try {
      ...
      if (!hasData) {
         ...
         // Do not throw an error here, let the client decide how to handle an empty result
      }
