 ...
 * This action is designed to be called from the client, which will then handle
...
export async function processReceiptAction(
    ...
    try {
        const input: ProcessReceiptInput = {
            // We provide a temporary receiptId, as it's required by the schema,
            // but the actual document will be created on the client.
